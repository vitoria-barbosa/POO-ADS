...
class TestesBancoLimite {
  ...
  private teste1Deposito(): void {
    this.rodarTeste(
      ...
      () => {
        ...
        this.assert(
          opRegistro.tipo == "CRÉDITO",
          "Operação deveria ser do tipo CRÉDITO."
        );
        this.assert(
          opRegistro.valor == 200,
          "Valor da operação deveria ser 200."
        );
        this.assert(
          opRegistro.conta == conta,
          "Operação deve estar ligada à conta correta."
        );
        this.assert(
          opRegistro == op,
          "Operação retornada pelo método deve ser a mesma registrada."
        );
      }
    );
  }
  ...
  private teste2SaqueComLimite(): void {
    this.rodarTeste(
      ...
      () => {
        ...
        this.assert(
          ops.length == 3,
          "Devem existir 3 operações registradas (incluindo falha)."
        );
        ...
        this.assert(
          maisRecente.tipo == "FALHA",
          "A operação mais recente deveria ser uma FALHA."
        );
        this.assert(
          segunda.tipo == "DÉBITO",
          "A segunda operação deveria ser DÉBITO."
        );
        this.assert(
          terceira.tipo == "DÉBITO",
          "A terceira operação deveria ser DÉBITO."
        );
  ...
  private teste3TransferenciaComLimite(): void {
    this.rodarTeste(
      ...
      () => {
        ...
        this.assert(
          ops1.length == 2,
          "Transferência válida deve gerar 2 operações."
        );
        this.assert(
          origem.saldo == -150,
          "Saldo da conta origem deveria ser -150."
        );
        this.assert(
          destino.saldo == 300,
          "Saldo da conta destino deveria ser 300."
        );

        const ops2 = origem.transferir(destino, 100);
        this.assert(
          ops2.length == 1,
          "Transferência recusada deve gerar apenas 1 operação."
        );
        const opFalha = ops2[0];
        this.assert(
          opFalha.tipo == "FALHA",
          "Tipo da operação deve ser FALHA."
        );
        this.assert(
          origem.saldo == -150,
          "Saldo da origem não deve mudar após falha."
        );
        this.assert(
          destino.saldo == 300,
          "Saldo do destino não deve mudar após falha."
        );
        ...
        this.assert(
          qtdFalhas == 1,
          "Deveria haver exatamente 1 operação de falha na origem."
        );

        const opsDestino = destino.operacoes;
        const qtdCreditosDestino = opsDestino.filter(
          (o) => o.tipo == "CRÉDITO"
        ).length;
        this.assert(
          qtdCreditosDestino == 1,
          "Destino deveria ter exatamente 1 crédito válido."
        );
      }
    );
  }
  ...
  private teste4BancoRegistroGeral(): void {
    this.rodarTeste(
      ...
      () => {
        ...
        const qtdFalhasConta1 = extratoConta1.filter(
          (o) => o.tipo == "FALHA"
        ).length;
        this.assert(
          qtdFalhasConta1 == 1,
          "Conta 301 deveria ter 1 operação de falha."
        );
  ...
  private teste5ClienteTresContas(): void {
    this.rodarTeste(
      ...
      () => {
        ...
        this.assert(
          c1.saldo == 200,
          "Saldo final da conta 401 deveria ser 200."
        );
        this.assert(
          c2.saldo == -30,
          "Saldo final da conta 402 deveria ser -30."
        );
  ...
  public executar(): void {
    ...
    console.log("=======================================");
    console.log(
      "Pontos obtidos: " + this._pontosObtidos + " / " + this._pontosTotais
    );
