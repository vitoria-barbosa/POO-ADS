...
export default class Banco {
  ...
  private _idClienteAtual = 1;
  private _idContaAtual = 1;

  public sacar(numero: string, valor: number): void {
    let contaProcurada: Conta | undefined = this.procurarConta(numero);

    if (contaProcurada) {
      let operacao = contaProcurada.sacar(valor);
  ...
  public depositar(numero: string, valor: number): void {
    ...
    if (contaProcurada) {
      let operacao = contaProcurada.depositar(valor);
      this.adicionarOperacao(operacao);
    }
  }
  ...
  public transferir(
    ...
    let contaOrigem: Conta | undefined = this.procurarConta(numeroOrigem);
    let contaDestino: Conta | undefined = this.procurarConta(numeroDestino);
  ...
  public consultarExtratoConta(numeroConta: string): Operacao[] {
    let conta = this.procurarConta(numeroConta);
    return conta.operacoes;
  }

  public consultarExtratoGeral(): Operacao[] {
    return this._operacoes;
  }

  public adicionarOperacao(operacao: Operacao) {
    this._operacoes.unshift(operacao);
  }

  public procurarCliente(CPFCliente: string): Cliente | undefined {
    return this._clientes.find((c) => c.cpf == CPFCliente);
  }

  public procurarConta(numConta: string): Conta | undefined {
    return this.contas.find((conta) => conta.numero == numConta);
  }

  public associarContaCliente(numeroConta: string, cpfCliente: string): void {
    let contaProcurada: Conta | undefined = this.procurarConta(numeroConta);
    let clienteProcurado: Cliente | undefined =
      this.procurarCliente(cpfCliente);
    ...
    if (
      ...
      contaProcurada.titular = clienteProcurado;
      clienteProcurado.contas.push(contaProcurada);
    }
  }

  public jaExisteContaParaCliente(cpf: string, numero: string): boolean {
    let contaProcurada = this.procurarConta(numero);
    let clienteProcurado = this.procurarCliente(cpf);

    if (!contaProcurada && !clienteProcurado) {
      return false;
    }

    if (contaProcurada?.titular == null) {
      return false;
    }

    if (contaProcurada.titular.cpf == clienteProcurado.cpf) {
      return true;
    }
  ...
  public cadastrarContaCliente(): void {
    let cliente: Cliente;
    let conta: Conta;
